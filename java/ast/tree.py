from java.ast.node import Node
...
class SynchronizationStatement(Statement):
    attrs = ("lock","block")
...
class TryResource(Declaration):
    atrs = ("type", "name", "value")

class CatchClause(Statement):
    attrs = ("parameter", "block")

class CatchClauseParameters(Declaration):
    attrs = ("types", "name")
...
class ArraySelector(Invocation):
    attrs = ("array",)
...
class AnnotationMethod(Declaration):
    attrs = ("name", "return_type", "dimensions", "defaults")