...
from utils.mutf8 import Mutf8String
...
class JavaClass(KaitaiStruct):
    ...
    class Utf8CpInfo(KaitaiStruct):
        ...
        def _read(self):
            ...
            raw_bytes = self._io.read_bytes(self.str_len)
            if len(raw_bytes) == 0:
                self.value = ""
            else:
                print(f"Num bytes: {len(raw_bytes)}, raw bytes: {raw_bytes}")
                mutf8_str = Mutf8String(raw_bytes)
                self.value = mutf8_str.decode()
            # try:
            #     self.value = decode_modified_utf8(raw_bytes)
            # except UnicodeDecodeError as e:
            #     # Fallback: if Modified UTF-8 fails, try to salvage what we can
            #     print(f"Warning: Modified UTF-8 decode failed at position {self._io.pos() - self.str_len}: {e}")
            #     print(f"  Raw bytes: {raw_bytes.hex()}")
            #
            #     # Try replacing invalid sequences
            #     self.value = raw_bytes.decode('utf-8', errors='replace')
            #
            #     # Or use Latin-1 as last resort (never fails)
            #     # self.value = raw_bytes.decode('latin-1')
