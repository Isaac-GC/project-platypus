...
BRANCH_OFFSET_BITS = {
    ...
    0x29: 16,  # goto/16
    0x2a: 32,  # goto/32
    0x32: 16, 0x33: 16, 0x34: 16,  # if-eq, if-ne, if-lt
    0x35: 16, 0x36: 16, 0x37: 16,  # if-ge, if-gt, if-le
    0x38: 16, 0x39: 16, 0x3a: 16,  # if-eqz, if-nez, if-ltz
    0x3b: 16, 0x3c: 16, 0x3d: 16,  # if-gez, if-gtz, if-lez
    0x2b: 32,  # packed-switch
    0x2c: 32,  # sparse-switch
    0x26: 32,  # fill-array-data
}

BRANCH_OFFSET_IDX = {
    **{op: 0 for op in (0x28, 0x29, 0x2a)},          # goto family: operands[0]
    **{op: 1 for op in (0x38, 0x39, 0x3a,             # if-*z: operands[1]
                         0x3b, 0x3c, 0x3d)},
    **{op: 2 for op in (0x32, 0x33, 0x34,             # if-*: operands[2]
                         0x35, 0x36, 0x37)},
    **{op: 1 for op in (0x2b, 0x2c, 0x26)},           # switch/array: operands[1]
}


def get_opcode_width(opcode: int) -> int:
    if opcode not in OPCODE_WIDTH:
...
def resolve_branch_target(instr) -> int:
    ...
    raw  = instr.operands[idx]

    return instr.codepoint + sign_extend(raw, bits)