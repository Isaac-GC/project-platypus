...
def Ljava_lang_String_0init0(params: list, vm, v: list):
        ...
        if v[params[0]] == '':
            ...
            try:
                val = bytearray(v[params[1]]).decode("utf-8")
                ...
                for b in val:
                    if b < 0:
                        b += 0xFF + 1
                    ret.append(b)
                val = bytearray(ret).decode("utf-8", "ignore")
            # print(v[params[0]])
            dump_string(f"{val}", vm)
            # log.info(f"String created: {val}")
            vm.memory.last_return = val
...
def Ljavax_crypto_spec_IvParameterSpec_0init0(params: list, vm, v: list):
    state_data['aes_iv'] = v[params[1]]

def Ljavax_crypto_Cipher_getInstance(params: list, vm, v:list):
    cipher_type = v[params[0]]
    if cipher_type == b'AES/CBC/PKCS5Padding':
        state_data['aes_cipher'] = 'aes_cbc_pkcs5'

from Crypto.Cipher import AES
from Crypto.Util.Padding import unpad

def Ljavax_crypto_Cipher_doFinal(params: list, vm, v:list):
    # print(v[params[1]]), print(len(v[params[1]]))
    key = bytearray(state_data['aes_key'])
    iv = bytearray(state_data['aes_iv'])
    cipher_text = bytearray(v[params[1]])

    cipher = None
    if state_data['aes_cipher'] == 'aes_cbc_pkcs5':
        cipher = AES.new(key=key, mode=AES.MODE_CBC, iv=iv)


    original_data = unpad(cipher.decrypt(cipher_text), AES.block_size)
    # print(original_data)
    # original_data = unpad(decrypted)
    # print(original_data.decode())
    # print("".join([b for b in original_data]))
